...
public class Arith {
	...
	public static double round(BigDecimal bigDecimal, int scale) {
		...
		if (null == bigDecimal) {
			throw new IllegalArgumentException("bigDecimal 不能为null");
		}
		if (scale < 0) {
	...
	public static boolean isZero(double value) {
		return isZero(value, 4);
	}

	public static boolean isZero(double value, int scale) {
		BigDecimal bigDecimal = new BigDecimal(value);
		return isZero(bigDecimal, scale);
	}
	...
	public static boolean isZero(BigDecimal value, int scale) {
		...
		if (scale < 0) {
			throw new IllegalArgumentException("精度为正整数");
		}

		BigDecimal bigDecimal = value.setScale(scale, RoundingMode.DOWN);
